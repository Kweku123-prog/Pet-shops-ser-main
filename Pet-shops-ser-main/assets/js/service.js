...
if($('.submit_status').val() == 0) {
let formData = new FormData($('#add_product')[0]), files = [];
for (var i = 0; i < AttachmentArray.length; i++) {
   // console.log(AttachmentArray[i].file);
    formData.append('images2[]', AttachmentArray[i].file);
}
$('#load_div').html('<img src="'+base_url+'assets/img/loader.gif" alt="" />');
 ...
 $('#submit_add_product').attr('disabled',true);
$.ajax({
    type: "POST",
    url: base_url+"user/AddProduct/add_product_ajax",
    data:formData,
    processData: false,
    contentType: false, 
    beforeSend: function() {
...
//document end
  ...
  function initialize() {
    ...
    function get_latitude_longitude() {
      ...
      if($('#add_service, #update_service,#add_product').length > 0 ){
